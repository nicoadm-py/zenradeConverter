...
| `zenradeconverter/spotify.py` | spotdl wrapper: fetch Spotify metadata, YouTube Music match, mutagen tag override |
| `zenradeconverter/workers.py` | `QThread` subclasses — non-blocking download and Spotify pipeline |

Flow: `main.py` → `zenradeconverter.gui` → `spotify.fetch_tracks()` / `downloader.download()` → yt-dlp → FFmpeg → mutagen tag patch.
